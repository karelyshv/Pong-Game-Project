...
unsigned int Enviados = 2 , i, A;    //Número de elementos del arreglo a enviar
  ...
  for(;;){
    ...
    switch (estado){
      ...
      case MEDIR:
        ...
        AD1_GetChanValue16(0,&iADC.u16[0]);		//Lee señal analógica(0,&iADC.u16);
        AD1_GetChanValue16(1,&iADC.u16[2]);

        D[0] = Bit1_GetVal();				//Lee señal digital 1
  		  D[1] = Bit2_GetVal();				//Lee señal digiral 2
        D[2] = Bit3_GetVal();				//Lee señal digital 3
  		  D[3] = Bit4_GetVal();   		//Lee señal digiral 4
